...
export const uploadService = {
  validateFile(file: File): ValidationResult {
    if (!isAcceptedType(file.type)) {
      return { isValid: false, error: 'Unsupported file type. Use CSV, JSON, or MP3.' };
    }
    const limit = getMaxSize(file.type);
    if (limit && file.size > limit) {
      return { isValid: false, error: 'File is too large for the selected type.' };
    }
    return { isValid: true };
  },
