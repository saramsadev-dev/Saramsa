...
def _get_existing_source_ids(analysis_repo, project_id):
    ...
    try:
        ...
        ).values_list("payload__source_id", flat=True)
...
def _store_feedback(analysis_repo, project_id, user_id, feedback_items):
    """Persist normalized Slack feedback items into Analysis (PostgreSQL)."""
    import uuid
    ...
    try:
        analysis_repo.save_analysis_data(doc)
    except Exception as e:
        logger.warning(f"Failed to store slack feedback batch: {e}")
    ...
    for item in feedback_items:
        ...
        try:
            analysis_repo.save_analysis_data(item_doc)
        except Exception as e:
