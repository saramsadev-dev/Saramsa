...
| `.docx` | Backend `file_extractors.extract_comments_from_docx` | One non-empty line = one comment. Paragraph boundaries and soft line breaks (Shift+Enter / `<w:br/>`) both produce comment splits. Text inside tables, including arbitrarily nested tables, is collected too. Corrupt / password-protected files are rejected with HTTP 400. DOCX whose inner zip would decompress to more than 50 MB is rejected as a zip-bomb defense. |
